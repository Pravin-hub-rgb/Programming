...
for (int i = 0; i < 5; i++)
{
    printf("Enter the each char :");
    scanf("%c",&ptr[i]);
}
for (int i = 0; i < 5; i++)
{
    printf("Printing  each value of array ptr :%c",ptr[i]);
    
}
//gives some error as at first ptr was initialized as 'int' later using as 'char' that's why but still program still works
